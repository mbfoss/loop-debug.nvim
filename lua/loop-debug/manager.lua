            ...
            session_data.repl_ctrl.set_input_handler(function(input)
                data_providers.evaluate_provider({
                    ...
                    if not data then
                        ...
                        session_data.repl_ctrl.add_output(ansi_red .. msg .. ansi_reset)
                    else
...
function M.track_new_debugjob(task_name, page_manager)
    ---import, cannot handle multiple jobs in parallel, the first one have to end first
...
---@return boolean
---@return string|nil
local function _process_debug_command(jobdata, command)
...
function M.debug_command(command, arg1)
    ...
    local ok, err = _process_debug_command(jobdata, command)
    if not ok then
